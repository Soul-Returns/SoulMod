...
- `features/fishing/FishingTracker` — always-on tracker for sea-creature catches + double-hooks. Sticky-flag attribution mirroring `SkyHanni/SeaCreatureManager.kt`: `It's a Double Hook!(?: Woot woot!)?` line sets `pendingDoubleHook = true`; the next catch line consumes it and publishes `SeaCreatureCaught(creature, doubleHook, duringFestival, ts)`. Interleaver lines (autopet, `> Your bottle of thunder has fully charged!`, `A Reindrake forms from the depths.`, blank lines) do not reset the flag — they can legitimately land between the hook line and the catch line. Any other unrelated server line clears the flag. The double-hook chat line is subject to chat-compactor `(N)` coalescing; decoded with the same `lastCompactedBase/Count/At` delta logic as `SeasoningTracker`. Counters: all-time (`PersistentStats`), session (in-memory on the singleton), festival (additive bucket persisted to stats.json — cleared on `FishingFestivalEvent.Started`, retained through `Ended` so the HUD can show just-ended numbers). Per-creature breakdowns (`doubleHooksByCreature`, `catchesByCreature`, festival variants) are collected but **not** surfaced in the HUD — reserved for future SPV / backend / web consumption.
- `features/fishing/FishingTrackerOverlay` — concrete [tracker overlay](#tracker-overlay-framework-guilibtracker) for fishing. Tabs: `Session` (in-memory per-creature maps on `FishingTracker`) and `Total` (persisted `catchesByCreature` / `doubleHooksByCreature`). Sort: Catches (default) or Double Hooks. Limit cycles 5 / 10 / 15 / All. Reset button calls `FishingTracker.resetSession()`. A separate small text block (`fishing_festival_sticker`) renders above the panel showing the festival countdown when active — keeps the panel content focused on per-creature data. Gated on `cfg.fishing.fishingHud.showHud()` + `cfg.fishing.fishingTracker.enableTracker()` + `SkyblockApi.isOnSkyblock`.
...
- **Persisted fields** (`PersistentStats.Data`): `doubleHooksAllTime`, `catchesAllTime`, `doubleHooksByCreature: Map<String, Long>`, `catchesByCreature`, `festivalStartAt`, `festivalDoubleHooks`, `festivalCatches`, `festivalDoubleHooksByCreature`, `festivalCatchesByCreature`. Maps are **immutable** (`Map<String, Long>`, not `MutableMap`) — replaced via `m + (k to v)` rather than mutated, so the save-time `Data.copy()` (shallow) is safe against concurrent writes.
