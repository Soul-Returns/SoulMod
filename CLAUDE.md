...
- `ui/hud/FishingHud` — per-creature panel. **Layout (top to bottom):**
  ...
  - **Horizontal divider**, **footer** (only inside container screens, since clicks don't fire on bare HUD): three rows —
    1. Filter row: `MultiSelectDropdown` (`.weight(2f)`) + **Show All** `Button` (`.weight(1f)`). Multi-select over every `SeaCreature.variant` from the catalog, friendly-cased (`LAVA_CRIMSON_ISLE` → `Lava Crimson Isle`). Trigger label = `"Filter"` when empty, `"Filter (N)"` when N variants are picked. `popupMaxHeight = 156f` (≈ 8 visible rows + scroll). Empty selection = unfiltered (no synthetic "All" row in the popup); the **Show All** button clears the set. Filter applies on `creature.variant`; creatures not in the catalog are dropped when any filter is active.
...
- HUD settings persisted at `config/soul/fishing_hud.json` (`tab`, `sort`, `scrollOffset`, `showCatches/DH/Cocoons`, `categories` — empty set = unfiltered). Dropdown-open flags (`sortDropdownOpen`, `columnDropdownOpen`, `categoryDropdownOpen`) live on the singleton as `@Volatile` transients — never persisted.
