...
4. **Report what's there.** After a successful copy, run `ls -1 <target>/soul-*.jar 2>/dev/null` and show the user the current set of installed `soul-*.jar` files so they can see which old versions still need to be cleaned up after they close the game.
